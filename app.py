...
def generate_quiz_from_text(text):
    ...
    for sentence in sentences:
        number_match = re.search(
            r"\d+(?:,\d+)*(?:\.\d+)?(?:%|°C| km| million| billion| Ly| Billion Yrs)?",
            sentence,
        )
        ...
        results.append(
            {
                ...
                "difficulty": (
                    "Advanced"
                    if mastery > 80
                    else ("Intermediate" if mastery > 50 else "Beginner")
                ),
            }
        )
...
def simulate_mission():
    ...
    for cmd in commands:
        ...
        if cmd == "F":
            ...
            move_message = "Analyzed area."
        else:
            move_message = f"Ignored unknown command: {cmd}"
        ...
        if cmd in ["F", "R"]:
            ...
            if mars_map[new_y][new_x] == 1:
                final_status = "Failed"
                final_message = (
                    f"Crashed into a rock at ({new_x}, {new_y}) after command {cmd}!"
                )
                path_log.append(
                    {
                        "command": cmd,
        ...
        path_log.append(
            {"command": cmd, "x": x, "y": y, "status": "OK", "message": move_message}
        )

    # Return the full log and the final summary
    return jsonify(
        {"status": final_status, "message": final_message, "path_log": path_log}
    )
